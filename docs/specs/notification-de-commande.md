...
**Amendement (2026-07-25).** User Story 8 et l'acceptance criterion du ticket 06 (« sans ouvrir la pièce jointe ») limitaient le corps de l'email restaurateur à nom du client / numéro de commande / Créneau. Sur demande du restaurateur, le corps de l'email gagne un **récapitulatif de la commande** — même contenu que le tableau Produit / Qté / Total du ticket PDF (chaque ligne Formule avec sa Sélection groupée en dessous) et le même **Total payé** en pied — plutôt que la ventilation Sous-total / TVA / Livraison de `order-confirmation.tsx`, qui reste propre à la Facture. La résolution de la Sélection (`resolveFormuleSelectionEntries`) est appelée une seconde fois côté template email, sur la Curation déjà résolue par le subscriber : ce n'est pas la duplication d'appel réseau que l'amendement du 2026-07-18 proscrit (`getFormuleCurationForVariant` reste appelé une seule fois par Variante), seulement un second passage sur des données déjà en mémoire. Code faisant foi : `templates/kitchen-ticket-notification.tsx`.
